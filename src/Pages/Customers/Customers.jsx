...
const Customers = () => {
    ...
    const optionRef = useRef(customers.map((item)=>React.createRef()));
  ...
  function handleProfile(e){
   console.log(profileRef.current && !profileRef.current.contains(e.target))
  ...
  <table className="table ">
...

    {
            customers.map((customer,index)=>{
                return<tr className=' bg-color_main text-text_main hover:bg-color_main_light hover:cursor-pointer ' key={index} ref={optionRef.current[index]}>
                ...
                <td className='relative' >
                   ...
                   <div className={`absolute top-10 z-40 font-semibold -left-20 min-w-24 rounded-lg flex flex-col justify-center  bg-color_main_light text-text_main p-2 shado shadow-lg ${index === activeOption ? 'block' : 'hidden'} `} ref={optionRef}>
                    <Link className='p-1 hover:bg-color_main' onClick={()=>setActiveIndex(index)}>Profile</Link>
                   {
         ...
         {
            activeIndex !== null &&   <div className='fixed lg:w-[calc(100%-20%)] w-full h-[100vh] flex justify-center items-center top-0 lg:left-[20%] left-0  lg:px-0 px-2  bg-black bg-opacity-40 '>

            <div className=' bg-color_main_light p-3 rounded-lg lg:w-1/2 w-full transition-all  ease-in duration-200  ' ref={profileRef}>
                  <div className='flex gap-2'>
                  <img src={'https://i.ibb.co/x7fZytf/IMG20230601121705.jpg'} className='w-14 h-14 rounded-lg'></img>
                  <div>
                      <h1 className='text-2xl text-text_main'>{customers[activeIndex]?.name}</h1>
                  ...
                  <div className='mt-3'>
                      ...
                      <div className='space-y-2 mt-2'>
                          ...
                          <h2>Contact Phone: {customers[activeIndex]?.contact_phone}</h2>
                          <h2>Joining Date: {customers[activeIndex]?.joining_date}</h2>
                          <h2>Ac Status: <span className='px-3 py-1 text-black text-center rounded-full' style={{backgroundColor:customers[activeIndex]?.status === 'Active' ? '#cef5de' : '#f1aeb4'}}>{customers[activeIndex]?.status}</span></h2>

                  </div>
                  <div className=' flex items-center justify-between mt-3 text-white '>
                  <button className=' text-color_success '>Set as star client</button>
                      <button className='text-[#e12d3c] '>Block {customers[activeIndex]?.name}</button>
                  </div>
              </div>
            </div>
          </div>
